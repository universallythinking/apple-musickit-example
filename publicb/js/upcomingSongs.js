$(document).ready(function() {
        ...
        for (var a = 0; a < t.items.length; a++) {
            if (t.items[a].track.id == localStorage.nowp || localStorage.nowp == hashCode(t.items[a].track.name + t.items[a].track.artists[0].name)) {
                try {
                    ...
                    $("#song").attr("data", t.items[a].track.id.substring(1,5));
            ...
            if (t.items[a].track.id != localStorage.nowp && t.items[a].track.id != null && t.items[a].track.id != "null") {
                $("#results").append('<div title="' + t.items[a].track.id + '" data="' + t.items[a].track.id.substring(1,5) + '" name="' + t.items[a].track.popularity + '" alt="' + t.items[a].track.name + " by " + t.items[a].track.artists[0].name + '" class="songLinkClick next played song-' + a + '" id="songLinkClick' + a + '"><div class="voteBtn voteDown"><i class="fa fa-thumbs-o-down" aria-hidden="true"></i></div> <div class="info"> <div class="img first"></div> <div class="titles"> <h5 onclick="now(\'' + t.items[a].track.id + '\', ' + a + ')"  class="block">' + t.items[a].track.name + '</h5> <p class="block">' + t.items[a].track.artists[0].name + '</p> </div> </div> <div class="voteUp voteBtn"><i class="fa fa-thumbs-o-up" aria-hidden="true"></i></div></div>  </div>');
            } else if (t.items[a].track.id != localStorage.nowp && (t.items[a].track.id === null || t.items[a].track.id == "null")) {
                $("#results").append('<div title="' + hashCode(t.items[a].track.name + t.items[a].track.artists[0].name) + '" data="' + hashCode(t.items[a].track.name + t.items[a].track.artists[0].name).substring(1,5) + '" name="' + t.items[a].track.popularity + '" alt="' + t.items[a].track.name + " by " + t.items[a].track.artists[0].name + '" class="songLinkClick next played song-' + a + '" id="songLinkClick' + a + '"><div class="voteBtn voteDown"><i class="fa fa-thumbs-o-down" aria-hidden="true"></i></div> <div class="info"> <div class="img first"></div> <div class="titles"> <h5 onclick="now(\'' + t.items[a].track.id + '\', ' + a + ')"  class="block">' + t.items[a].track.name + '</h5> <p class="block">' + t.items[a].track.artists[0].name + '</p> </div> </div> <div class="voteUp voteBtn"><i class="fa fa-thumbs-o-up" aria-hidden="true"></i></div></div>  </div>');
            }
            if (t.items[a].track.id != null && t.items[a].track.id != "null") {
                l2.push(t.items[a].track.id.substring(1,5));
            } else if ((t.items[a].track.id === null || t.items[a].track.id == "null")) {
                l2.push(hashCode(t.items[a].track.name + t.items[a].track.artists[0].name).substring(1,5));
            }
        ...
        l = localStorage.CT.split(","),
            $.each(l, function(e, n) {
                t = localStorage.votes.split("++" + n).length,
                    ...
                    t - a != 0 ? ($("[data=" + n + "]").children(".voteUp").text(t - a),
                        $("[data=" + n + "]").children(".voteUp").attr("name", t - a)) : ($("[data=" + n + "]").children(".voteUp").html("<i class='fa fa-thumbs-o-up' aria-hidden='true'></i>"),
                        $("[data=" + n + "]").children(".voteUp").attr("name", t - a))
            });
    ...
    window.down = function() {
        var t = localStorage.nowp.substring(1,5),
            a = new RegExp(t, "g"),
            n = new RegExp("", "g"),
    ...
    //setTimeout(smallVote, 15000);
     window.moveToBottom = function() {
                    ...
                    var add = [],
                        l = localStorage.CT.split(",");
                    $.each(l, function(t, l) {
                        e = localStorage.votes.split("++" + l).length, a = localStorage.votes.split("--" + l).length, $("[data=" + l + "]").attr("title"), (e - a <= o && localStorage.nowp.indexOf(l) == -1) && s.push('{ "uri": "spotify:track:' + $("[data=" + l + "]").attr("title") + '" }')
                    });
                     $.each(l, function(t, l) {
                        e = localStorage.votes.split("++" + l).length, a = localStorage.votes.split("--" + l).length, $("[data=" + l + "]").attr("title"), (e - a <= o && localStorage.nowp.indexOf(l) == -1) && add.push('"spotify:track:' + $("[data=" + l + "]").attr("title") + '"')
                    });
                    var addObj = '{"uris":[' + add + ']}';
