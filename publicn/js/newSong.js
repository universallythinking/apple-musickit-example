$(document).ready(function() {
	window.hashCodeNum = function (str) {
		var hash = 0,
			i, chr;
	...
	if (!localStorage.hashNum) localStorage.hashNum = hashCodeNum(new Date().getTime().toString());
	...
	window.recon = function () {
		...
		socket.on(localStorage.userID, function (data) {
        	...
        	socket.on(localStorage.userID + 'updateios', function (data) {
        		...
        		if(document.hidden)setTimeout(calculateVotes,5000);
        		if(!document.hidden)calculateVotes();
        	...
        	socket.on(localStorage.userID + 'newSong', function (data) {
        		...
        		setTimeout(nextSongs, 8000);
        		console.log("New Song");

        	});
        	socket.on('disconnect', function () {
        		setTimeout(function () {
                        		if (!socket.connected) recon();
                	}, 3000);
        	});
	...
	window.resetVotes = function () {
		if (1 == 1) {
			...
			var a = {};
			a.username = localStorage.userID, $.ajax({
				url: "https://paywallios.herokuapp.com/clearVotes",
				dataType: "json",
				data: a,
				success: function (a) {
					localStorage.STOP = "false", localStorage.votes = "";
					location.href = location.href;
				}
			})
		}
