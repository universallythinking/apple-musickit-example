...
window.newSongPlayed = function(t) {
    ...
    var t = [],
       ...
       if($(".removeSong").length <= 0) {
        localStorage.image = e.relationships.tracks.data[0].attributes.artwork.url;
           //$("#results").css("padding-top", "298px !important"), $("#results").css("text-align", "center"), $("#results").empty(), localStorage.nowp || null == e.relationships.tracks.data[0].id || "null" == e.relationships.tracks.data[0].id ? localStorage.nowp || null != e.relationships.tracks.data[0].id && "null" != e.relationships.tracks.data[0].id || (localStorage.nowp = hashCode(e.items[0].track.name + e.items[0].track.artists[0].name)) : localStorage.nowp = e.relationships.tracks.data[a].id;
    for (var a = 0; a < e.relationships.tracks.data.length; a++) {
        ...
        if (e.relationships.tracks.data[a].id.slice(2)== localStorage.nowp) try {
            $("#song").text(e.relationships.tracks.data[a].attributes.name), $("#song").attr("title", e.relationships.tracks.data[a].id), $("#song").attr("data", e.relationships.tracks.data[a].id.substring(1, 5)), $("#artist").text(e.relationships.tracks.data[a].attributes.artistName), $(".img").eq(a).css("background", "url(" + image + ") center center"), $(".img").eq(a).css("background-size", "cover"), $("#albumImage").css("background", "linear-gradient(rgba(0, 0, 0, 0.3), rgba(0, 0, 0, 0.4)), url(" + image + ") center top")
        } catch (err) {}
        e.relationships.tracks.data[a].id.slice(2)!= localStorage.nowp && null != e.relationships.tracks.data[a].id.slice(2)&& "null" != e.relationships.tracks.data[a].id.slice(2)? ($("#results").append('<div title="' + e.relationships.tracks.data[a].id.slice(2) + '" data="' + e.relationships.tracks.data[a].id.substring(1, 5) + '" class="songLinkClick next played song-' + a + '" id="songLinkClick' + a + '"> <div class="info"> <div class="img first"></div> <div class="titles"> <h5 onclick="now(\'' + e.relationships.tracks.data[a].id.slice(2) + "', " + a + ')" class="block">' +e.relationships.tracks.data[a].attributes.name + '</h5> <p class="block"><span class="artistName">' + e.relationships.tracks.data[a].attributes.artistName + '</span> <span class="albumName">&#183; ' + e.relationships.tracks.data[a].attributes.albumName + '</span></p> </div> <div class="buttons"><div class="voteBtn voteDown"><i class="fa fa-thumbs-o-down" aria-hidden="true"></i></div><div data="' + a + '" class="voteUp voteBtn" name="0"><i class="fa fa-thumbs-o-up" aria-hidden="true"></i></div><sup></sup></div></div></div>'), null != e.relationships.tracks.data[a].id.slice(2)&& "null" != e.relationships.tracks.data[a].id.slice(2)? t.push(e.relationships.tracks.data[a].id.slice(2)) : null !== e.relationships.tracks.data[a].id.slice(2)&& "null" != e.relationships.tracks.data[a].id.slice(2)|| t.push(e.relationships.tracks.data[a].id.slice(2))) : $("#results").append('<div title="' + hashCode(e.relationships.tracks.data[a].attributes.name + e.relationships.tracks.data[a].attributes.artistName + '" data="' + e.relationships.tracks.data[a].id.substring(1, 5) + '" class="songLinkClick next played song-' + a + '" id="songLinkClick' + a + '"> <div class="info"> <div class="img first"></div> <div class="titles"> <h5 class="block">' +e.relationships.tracks.data[a].attributes.name + '</h5> <p class="block"><span class="artistName">' + e.relationships.tracks.data[a].attributes.artistName + '</span> <span class="albumName">&#183; ' + e.relationships.tracks.data[a].attributes.albumName + '</span></p> </div><div class="buttons"><div class="voteBtn voteDown"><i class="fa fa-thumbs-o-down" aria-hidden="true"></i></div><div data="' + a + '" class="voteUp voteBtn" name="0"><i class="fa fa-thumbs-o-up" aria-hidden="true"></i></div><sup></sup></div></div></div>'), null != e.relationships.tracks.data[a].id.slice(2)&& "null" != e.relationships.tracks.data[a].id.slice(2)? t.push(e.relationships.tracks.data[a].id.slice(2)) : null !== e.relationships.tracks.data[a].id.slice(2)&& "null" != e.relationships.tracks.data[a].id.slice(2)|| t.push(e.relationships.tracks.data[a].id.slice(2)));
    }
       addVotes(),
    addSwipe(),
    localStorage.CT = t,
...
window.addVotes = function(e) {
    ...
    return t + e
           ...
           var a = {};
           localStorage.setItem("votes", localStorage.votes.toString() + t), calculateVotes(), a.votes = t, a.username = localStorage.userID, $.ajax({
               async: !1,
...
}, setTimeout(playFirst, 4000), window.next = function() {
    ...
    localStorage.playlistData ? nextSongsCache() : nextSongs()
}, setTimeout(nS, 2000), window.downVoteify = function(t) {
...
window.closeWin = function() {
    ...
    localStorage.hosting || (localStorage.hosting = 0, $("#hosting-toggle").text("ON"), $("#hosting-toggle").css("color", "green"), setTimeout(create, 5e3)), 0 == parseInt(localStorage.hosting) && $("#hosting-toggle").text("") ? ($("#hosting-toggle").text("ON"), $("#hosting-toggle").css("color", "green")) : 1 == parseInt(localStorage.hosting) && $("#hosting-toggle").text("") && ($("#hosting-toggle").text("OFF"), $("#hosting-toggle").css("color", "red"))
          ...
          var c = confirm("Create A New Playlist Next Time You Host?");
          if(c) {
          $.ajax({
              async: !0,
              type: "PUT",
              ...
              dataType: "json",
              data: JSON.stringify({
                  name: "Swagger Music - Saved"
              }),
              success: function(t) {
                 Cookies.remove('locStore');
                 localStorage.lStore = "true";
                 setTimeout(function() {
                   location.href = localStorage.urlParts + "/index.html";
                 }, 500);
                 }, error: function(e) {
...
window.removeHTML = function() {
    ...
    var urlParts = localStorage.urlParts;
    setTimeout(function() { location.href = urlParts + "/app2.html"; }, 250);
    localStorage.clear();
...
Cookies.set("locStore2", JSON.stringify(localStorage), {
            expires: 100
            }), localStorage.lStore ? 0 : Cookies.set("locStore", JSON.stringify(localStorage), {
            expires: 100
            }), setTimeout(function() {
...
window.cashOut = function() {
               ...
               e.preventDefault();
                              ...
                              $("#showPlaylists").on("tap", function(e) {
                              e.preventDefault();
                                                     $(".removeSong").remove();
                                                     playerToken();
                                                     $(".main").hide();
                              ...
                              $("#defaultTab").on("tap", function(e) {
                              e.preventDefault();
                                                  $('#playerResults').scrollTop(0);
                                                  setTimeout(function() {
                                                    $(window).scrollTop(0);
                                                   }, 5);
                                                  $(".removeSong").remove();
