//$(document).ready(function() {
    ...
    window.loadLists = function() {
        setTimeout(function() {
            ...
            data = data.playlists;
                arr = [];
                $("#Home").append("<header style='color: white !important; pointer-events: all; margin-bottom: 50px; margin-top: -25px;' class='songLinkClick1 items'><div><center class='items' style='font-size: 200%; padding-bottom: 50px; padding-top: -50px; '>Spotify Playlists</center></div></header><h1></h1><h1></h1>");
        ...
        setTimeout(function() {
            if (localStorage.userID) {
                $.ajax({
                    ...
                    data: "formdata",
                    success: function(data) {
                        $("#Home").append("<header style='color: white !important; pointer-events: all; margin-bottom: 50px; margin-top: -150px;' class='songLinkClick1 items playlist'><div><center class='items' style='font-size: 200%; padding-top: 50px; padding-bottom: 50px;'>Your Playlists</center></div></header>");
                        if (localStorage.Snapster) {
                            $("#Home").append("<div name='currentPlaylist' onclick='stay();' class='playlists items inline pSnap'><div class='charlie'><div class='row'><div><div><div><div><img class='images' src='dirtyScripts/turtle.png'/></div></div><div class='playlist-desc'><h3</h3><h3></h3><h5>Current Playlist</h5><p>...</p></div></div></div>");
                        }
                        for (var i = 0; i < data.items.length; i++) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if(localStorage.explicit.toString() != "false") {
                    for (var i = 0; i < Math.min(data.items.length, 75); i++) {
