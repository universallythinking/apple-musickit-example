...
i = [], window.nextSongs = function() {
    ...
    if (1 == e) {
        ...
        vote(a, o)
    } else 0 == e && (a = "--" + $("#results").find("#songLinkClick" + t).attr("title"), vote(a, 0))
}, localStorage.votes || (localStorage.votes = ""), window.calculateVotes = function() {
console.log("CV1");
    var t, e;
    l = localStorage.CT.split(","), $.each(l, function(a, o) {
        t = localStorage.votes.split("++" + o).length, e = localStorage.votes.split("--" + o).length, t - e != 0 ? ($("[title=" + o + "]").children(".voteUp").text(t - e), $("[title=" + o + "]").children(".voteUp").attr("name", t - e), $("[title=" + o + "]").children(".voteUp").css("fontWeight", "bold")) : ($("[title=" + o + "]").children(".voteUp").html("<i class='fa fa-thumbs-o-up' aria-hidden='true'></i>"), $("[title=" + o + "]").children(".voteUp").attr("name", t - e))
    ...
    var a = {};
    localStorage.setItem("votes", localStorage.votes.toString() + t), setTimeout(calculateVotes,150), a.votes = t, a.username = localStorage.userID, $.ajax({
        async: !1,
    ...
    if (parseInt($(".songLinkClick").eq(0).children(".voteUp").attr("name")) < 0 && resetVotes(), parseInt($(".songLinkClick").last().children(".voteUp").attr("name")) < t) {
        ...
        $.each(l, function(t, l) {
            e = localStorage.votes.split("++" + l).length, a = localStorage.votes.split("--" + l).length, $("[title=" + l + "]")[0].id.slice(13), e - a <= o && s.push('{ "uri": "spotify:track:' + l + '" }')
        });
        var i = '{"tracks":[' + s + "]}";
...
}, $("#Home").children.length < 10 && setTimeout(loadLists, 5e3), setTimeout(playLoader, 5e3), localStorage.combo = localStorage.userID + ":::" + localStorage.password, WEB_SOCKET_SWF_LOCATION = "https://socketswagger.herokuapp.com/socket.io/inc/WebSocketMain.swf", hashCodeNum = function(t) {
    var e, a = 0;
    if (0 === t.length) return a;
    for (e = 0; e < t.length; e++) a = (a << 5) - a + t.charCodeAt(e), a |= 0;
    return a
...
t.combo = localStorage.combo, t.username = localStorage.userID, t.hash = localStorage.hashNum, $.ajax({
    ...
    if(10 < t.body.length && localStorage.votes != t.body) { localStorage.votes = t.body; setTimeout(calculateVotes, 1500); console.log("VOTES " + t.body); }
    ...
    localStorage.Snapster != t && t.length
...
localStorage.setItem("guest", !0), localStorage.setItem("temp", !0), localStorage.setItem("allowed", !0), window.start = function() {
    ...
    localStorage.urlParts, localStorage.clear(), location.href = "//partymusic://terminate", localStorage.clear()
}, window.softTerminate = function() {
    Cookies.remove("locStore"), Cookies.remove("justClicked"), localStorage.urlParts, localStorage.clear(), setTimeout(function() {
